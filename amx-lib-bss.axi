...
// 12 characters in body that can be escaped (24 bytes), + msg type,
// STX, ETX, checksum (2 bytes escaped).
BSS_MAX_PACKET_LEN = 29;
...
// Message acknowledgement mode. Off for TCP, on for serial.
char bssMsgAck = BSS_MSG_ACK_OFF;
...
define_function _bssSend(char body[])
{
    ...
    // Escape special characters in payload data and wrap with STX, checksum,
    // and ETX bytes.
    packet = "BSS_STX";
